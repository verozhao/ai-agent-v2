...
class TestFeedbackLoop:
    ...
    async def test_pattern_reinforcement(self, agent, feedback_loop):
        ...
        for i in range(5):
            corrupted = {"fund_name": f"2019-0{i+1}-15", "date": f"Fund ABC {i}"}

            await feedback_loop.process_document(corrupted, f"doc_{i}")

            # Validate each one
            ground_truth = {"fund_name": f"Fund ABC {i}", "date": f"2019-0{i+1}-15"}
        ...
        new_corrupted = {"fund_name": "2019-07-20", "date": "Fund XYZ"}
    ...
    async def test_learning_from_mistakes(self, agent, feedback_loop):
        ...
        similar = {"fund_name": "Venture Capital Fund", "date": "Invalid", "amount": "$2,000,000"}
